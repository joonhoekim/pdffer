  ...
  return (
    <div className="text-sm">
      <div className="flex items-center gap-2 hover:bg-gray-100 p-2 rounded">
        ...
        <span className="text-xs text-gray-500">{(entry.file.size / 1024).toFixed(1)}KB</span>
        {getStatusIcon()}
      </div>
    </div>
  );
...
export default function StorageUploadPage() {
  ...
  const updateFileStatus = (fileName: string, status: FileEntry["uploadStatus"]) => {
    setFileEntries((prev) => prev.map((entry) => (entry.name === fileName ? { ...entry, uploadStatus: status } : entry)));
  };
    ...
    <Card className="w-full max-w-2xl mx-auto">
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <Button onClick={handleUpload} disabled={fileEntries.length === 0} className="w-full">
            {fileEntries.length > 0 ? `${fileEntries.length}개 파일 업로드` : "파일을 선택하세요"}
          </Button>
